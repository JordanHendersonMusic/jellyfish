...
#region Private members
    ...
    private TentaOptions[] _tOpts = new TentaOptions[1];
...
#region VFX
    ...
    private void SetTextureVelocity(OSCMessage msg){
        if (msg.ToVector2(out Vector2 v)){
            texture_position += v * 2 - (Vector2.one);
            _vfx.SetVector2("textureOffSet", texture_position);
        }
    }
#endregion VFX

#region Compute
    Vector3 _smallMovementPosition;
    public Transform StartPosition;
    public Transform AbovePosition;
    private Vector3 _currentLargePosition = new Vector3();
    ...
    private void SetWidth(OSCMessage msg) {
        if (msg.ToFloat(out var f)) {
            _tOpts[0].width = f * 10.0f + 0.1f;
        }
    }

    private void SetNoiseScale(OSCMessage msg) {
        if (msg.ToVector2(out var s)) {
    ...
    private void SetTensionForce(OSCMessage msg) {
        if (msg.ToFloat(out var f)) {
            _tOpts[0].tensionStrength = f * 35f;
        }
    }

    private void SetDecay(OSCMessage msg) {
        if (msg.ToFloat(out var f)) {
            _tOpts[0].velocityDecay = f * 0.925f;
        }
    }
